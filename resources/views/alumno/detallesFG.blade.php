...
<div class="container mx-auto px-4">
    @php
    $usuario = Auth::guard('usuarios_alumno')->user();
    @endphp
    <div class="container mx-auto px-4 mt-6">
        <div class="mb-6">
            <a href="{{ url()->previous() }}" 
            class="inline-block px-5 py-2 bg-gray-300 text-gray-800 rounded-lg shadow hover:bg-gray-400 transition">
            ⬅️ Regresar
            </a>
        </div>
    </div>
  ...
  <div class="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-md">
    ...
    <form action="/alumno/focus-group/enviar-mensaje" method="POST">
      @csrf
      ...
      <input type="hidden" name="focus_group_id_focus_group" value="{{ $focusGroup->id_focus_group }}">
      <input type="hidden" name="nombre_usuario" value="{{ $usuario->alumno->nombre }} {{ $usuario->alumno->apellido_paterno }} {{ $usuario->alumno->apellido_materno }}">
